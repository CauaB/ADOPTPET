...
$sqlEndereco = "SELECT endereco, COUNT(*) as quantidade FROM adotante GROUP BY endereco";
$resultadoEndereco = mysqli_query($conn, $sqlEndereco);

$enderecos = [];
$quantidadeAdotantes = [];
$quantidadeOutros = 0;

while ($row = mysqli_fetch_assoc($resultadoEndereco)) {
    $endereco = $row['endereco'];
    $quantidade = $row['quantidade'];

    // Capitaliza a primeira letra de cada palavra no endereço
    $enderecoCapitalizado = ucwords(strtolower($endereco));

    // Verifica se o endereço contém as palavras-chave de forma insensível a maiúsculas e minúsculas
    if (stripos($enderecoCapitalizado, 'Loanda') !== false ||
        stripos($enderecoCapitalizado, 'Carneirinhos') !== false ||
        stripos($enderecoCapitalizado, 'Cruzeiro') !== false ||
        stripos($enderecoCapitalizado, 'Bau') !== false) {
        $enderecos[] = $enderecoCapitalizado;
        $quantidadeAdotantes[] = $quantidade;
    } else {
        // Adiciona a quantidade ao grupo "Outros"
        $quantidadeOutros += $quantidade;
    }
}

// Adiciona o grupo "Outros" aos arrays
if ($quantidadeOutros > 0) {
    $enderecos[] = 'Outros';
    $quantidadeAdotantes[] = $quantidadeOutros;
}
...
<nav class="navbar navbar-expand-lg navbar-custom">
  ...
  <div class="container">
    <h3>Pesquisa de Pets Adotados</h3>
  ...
  <br>
    <div class="dashboard-section">
      <h3>Dashboard de Pets</h3>
      <div class="row">
        <div class="col-md-6">
          <h4>Gráfico de Pets por Espécie</h4>
          <canvas id="graficoEspecie"></canvas>
        </div>
        <div class="col-md-6">
          <h4>Gráfico de Pets por Raça</h4>
          <canvas id="graficoRaca"></canvas>
        </div>
        <br>
        <div class="col-md-12">
          <h4>Gráfico de Adotantes por Endereço</h4>
          <canvas id="graficoEndereco"></canvas>
        </div>
      </div>
    </div>
  </div>
...
<script>
    ...
    var ctxEndereco = document.getElementById('graficoEndereco').getContext('2d');
    var graficoEndereco = new Chart(ctxEndereco, {
        type: 'bar',
        data: {
            labels: <?php echo json_encode($enderecos); ?>,
            datasets: [{
    ...
    scales: {
        x: {
            beginAtZero: true,
            ticks: {
                maxRotation: 45,  // Rotaciona os rótulos do eixo x para melhor visualização
                minRotation: 0
            }
        },
        y: {
            beginAtZero: true,
            ticks: {
                stepSize: 1,  // Define o incremento para números inteiros sem repetição
                callback: function(value) {
                    return Math.round(value);  // Exibe números inteiros no eixo y
                }
            }
        }
    }
